...
static float fac_us_tmp=0;				//当时钟分频后不为整数时，精确延时  例如180/8=22.5 	比原来直接整除精度明显提高
...
void delay_init(u8 SYSCLK)
{
	...
	fac_us=SYSCLK/8;	
	fac_us_tmp=SYSCLK/8.0;						//不论是否使用OS,fac_us都需要使用
#if SYSTEM_SUPPORT_OS 						//如果需要支持OS.
...
#else
	fac_ms=(u16)(fac_us_tmp*1000);				//非OS下,代表每个ms需要的systick时钟数   
#endif
}								    
...
//延时nus
//nus为要延时的us数.	
//注意:nus的值,不要大于745654us(最大值即2^24/fac_us@fac_us=18000000/8=22.5)
void delay_us(u32 nus)
{		
	u32 temp;	    	 
	SysTick->LOAD=(u32)(nus*fac_us_tmp); 				//时间加载	  		 
...
void delay_ms(u16 nms)
{	 	 
	u8 repeat=nms/745;						//不考虑超频，已验证745ms准确，延时750ms仍为745ms
